...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!payload.poem || !payload.draftStages) {
      return res.status(400).json({ ok: false, error: 'Missing poem or draftStages' });
    }

    const supabase = getSupabase();
    const row = {
      poem: payload.poem,
      preview_line: payload.line || '',
    ...
    return res.status(200).json({ ok: true, id: data.id, letter: data });
  } catch (err) {
